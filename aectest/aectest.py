...
def repack_grib(input_file, output_file, packing_type):
    ...
    if( abs(orig_min-packed_min) > 0 or abs(orig_max-packed_max) > 0 or abs(orig_mean-packed_mean) > 0):
            err_msg = "Repacking yields different min,max or mean!."
            info(err_msg)
#            raise RuntimeError(err_msg)

    minmaxmean_values = [orig_min, orig_max,orig_mean,packed_min,packed_max,packed_mean]
...
def repack_and_check(input_dir, target_packing_type ):
    ...
    filelist = [ join(input_dir,f) for f in listdir(input_dir) if f.endswith("."+target_packing_type) or f.endswith(".reverted") ]
    if len(filelist) > 0:
        for f in filelist:
            os.remove(f)
   
    gribfiles = [join(input_dir,f) for f in listdir(input_dir) if isfile(join(input_dir, f))]
    ...
    for input_file in gribfiles:
        ...
        print("Repack "+input_file+" to "+target_packing_type+" & revert to "+original_encoded_type)
...
if __name__ == "__main__":
    ...
    for gribfile in gribfiles:
        ...
        retval = repack_and_check(gribfile, "grid_ccsds")
